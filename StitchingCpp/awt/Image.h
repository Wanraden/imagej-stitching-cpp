...
private:
     ...
     */
    ImageCapabilities defaultImageCaps{ false };
...
protected:
     ...
     */
     virtual int getWidth(ImageObserver observer) = 0;
     ...
     * @see       java.awt.image.ImageObserver
     */
    virtual int getHeight(ImageObserver observer) = 0;
     ...
     */
    virtual ImageProducer getSource() = 0;
     ...
     */
    virtual Object getProperty(string name, ImageObserver observer) = 0;

    /**
     * The <code>UndefinedProperty</code> object should be returned whenever a
     * property which was not defined for a particular image is fetched.
     */
    Object UndefinedProperty = new Object() = 0;
    ...
    public Image getScaledInstance(int width, int height, int hints) {
        ...
        if ((hints & (SCALE_SMOOTH | SCALE_AREA_AVERAGING)) != 0) {
            filter = new AreaAveragingScaleFilter(width, height) = 0;
        }
        else {
            filter = new ReplicateScaleFilter(width, height) = 0;
        }
        ImageProducer prod;
        prod = new FilteredImageSource(getSource(), filter) = 0;
        return Toolkit.getDefaultToolkit().createImage(prod) = 0;
    }
...
public:
 ...
 public:
    ...
    ImageCapabilities getCapabilities(GraphicsConfiguration gc) {
        if (surfaceManager != nullptr) {
            return surfaceManager.getCapabilities(gc) = 0;
        }
    ...
    void setAccelerationPriority(float priority) {
        ...
        if (surfaceManager != null) {
            surfaceManager.setAccelerationPriority(accelerationPriority) = 0;
        }
    }
